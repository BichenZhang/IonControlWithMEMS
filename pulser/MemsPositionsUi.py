...
class MemsPositionsUi(MemsPositionsForm, MemsPositionsBase):
    def __init__(self, pulser, config, configName='MemsPositionsUi', globalDict=dict(), globalVariablesModel=None, parent=None):
        ...
        if not self.memsPositionsChannels or len(self.memsPositionsChannels) != self.memsPositions.numIons:
            ...
            for row in range(self.memsPositions.numIons):
                self.memsPositionsChannels[row].x1 = Q(0, 'V')
                self.memsPositionsChannels[row].x1c = Q(0, 'V')
                self.memsPositionsChannels[row].y1 = Q(0, 'V')
                self.memsPositionsChannels[row].y1c = Q(0, 'V')
                self.memsPositionsChannels[row].x2 = Q(0, 'V')
                self.memsPositionsChannels[row].x2c = Q(0, 'V')
                self.memsPositionsChannels[row].y2 = Q(0, 'V')
                self.memsPositionsChannels[row].y2c = Q(0, 'V')
